...
let manifest: any
const components = Object.keys(modules).reduce<Record<string, () => Promise<unknown>>>((prev, curr) => {
  const formatPath = curr.replace('/src/pages', '')
  prev[formatPath] = async () => {
    try {
      return await modules[curr]()// 动态加载js
    } catch (error) {
      if (manifest) {
        try {
          // 有可能manifest是过期的，所以可能还会加载失败
          return await import( /* @vite-ignore */ '/' + manifest[`src/pages${formatPath}`]?.file);
        } catch {
