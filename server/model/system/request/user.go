...
type UserItem struct {
	...
	DeptId   uint   `json:"dept_id" binding:"required"`                                                      // 归属部门
	Phone    string `json:"phone"`                                                                           // 手机号
	Email    string `json:"email"`                                                                           // 邮箱
	Username string `json:"username" binding:"required,min=4,max=8,alphanum,startWithLetter,containsLetter"` // 用户名
	Sex      int    `json:"sex"`                                                                             // 性别
	PostId   uint   `json:"post_id,omitzero" binding:"required"`                                             // 岗位
	RoleId   uint   `json:"role_id,omitzero" binding:"required"`                                             // 角色
