...
func (s *AuthRouter) InitAuthRouter(r *gin.RouterGroup) gin.IRoutes {
	router := r.Group("auth")
	{
		...
		router.POST("select_tenant", middleware.IpRateLimitMiddleware(10*time.Second, 5), authApi.SelectTenant)                // 多租户登录选择租户
		router.GET("tenants", middleware.IpRateLimitMiddleware(10*time.Second, 5), authApi.AvailableTenants)                   // 可用租户列表（兼容保留）
		...
		router.Use(middleware.AuthMiddleware(), middleware.ContextMiddleware()).
			GET("logout", authApi.Logout).               // 退出登录
			POST("switch_tenant", authApi.SwitchTenant). // 切换租户
			GET("my_tenants", authApi.MyTenants)         // 我的可用租户
	}
	return router
}
