...
export function LunarCalendar() {
  ...
  return (
    <div className='bg-background grid h-full w-full grid-rows-[auto_1fr] rounded-xl font-sans shadow-sm select-none'>
      {/* 星期栏 */}
      <div className='bg-muted grid grid-cols-7 h-16'>
        {weekDays.map((w, i) => (
          <div
            key={w}
            className={`flex items-center justify-center h-full text-base font-bold ${i === 5 || i === 6 ? 'text-destructive' : 'text-muted-foreground'}`}
          >
            {w}
          </div>
        ))}
      ...
      <div className="relative h-full w-full">
        <AnimatePresence mode="wait" initial={false}>
          <motion.div
            ...
            className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 text-9xl font-bold z-1 whitespace-nowrap pointer-events-none select-none"
            initial={{ opacity: 0, x: direction.current === 0 ? 0 : direction.current > 0 ? 40 : -40 }}
            animate={{ opacity: 0.05, x: 0 }}
            exit={{ opacity: 0, x: direction.current === 0 ? 0 : direction.current > 0 ? -40 : 40 }}
            ...
            {rows.flat().map((cell, idx) =>
              cell ? (
                cell.isToday ? (
                    ...
                    <div className='text-primary-foreground flex min-h-4 w-full items-center justify-center gap-x-2 text-xs'>
                      {cell.isOtherMonth ? (
                        ...
                        <>
                          ...
                          <span>
                            {cell.lunar.ganzhi ? cell.lunar.ganzhi.slice(0, 2) : ''}
                          </span>
                        </>
                      )}
                    </div>
                  ...
                  <div
                    key={idx}
                    className={`group relative flex h-full min-h-[64px] w-full flex-col items-center justify-center px-1 py-2 transition-all ${getCellBgClass(cell)} ${cell.solar.week === 6 || cell.solar.week === 0
                      ? cell.isOtherMonth
                  ...
                  >
                    ...
                    <div
                      className={`flex min-h-4 w-full items-center justify-center gap-x-2 text-xs ${cell.isOtherMonth
