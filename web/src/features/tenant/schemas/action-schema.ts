...
export const getTenantCodeRule = () =>
  z
    ...
    .trim()
    .min(2, { error: t`租户编码长度不能小于2个字符` })
    .max(20, { error: t`租户编码长度不能大于20个字符` })
    .regex(/^[a-zA-Z0-9]+$/, { error: t`租户编码只能包含字母和数字` })
...
export const getSchema = () =>
  z
    .object({
      ...
      remark: z.string().max(200, { error: t`备注长度不能超过200个字符` }).optional(),
